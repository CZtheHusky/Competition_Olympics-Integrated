...
from ..tools.settings import COLORS, COLOR_TO_IDX, IDX_TO_COLOR
...
class curling_competition(OlympicsBase):
            ...
            for object_idx in range(len(self.map['objects'])):
                ...
                if not object.can_pass():
                    ...
                    if object.color == 'red' and object.type == 'cross' and \
                        ...
                        self.gamma = 1 - (
                                    1 - self.down_area_gamma) * self.faster  # this will change the gamma for the whole env, so need to change if dealing with multi-agent
                        self.release = True
    ...
    def _draw_curling_view1(self, obs, agent_list):  # obs: [2, 100, 100] list
        ...
        for agent_idx in range(len(agent_list)):
            ...
            pygame.draw.lines(self.viewer.background,
                              points=[[563 + 70 * count, 10], [563 + 70 * count, 70], [565 + 60 + 70 * count, 70],
                                      [565 + 60 + 70 * count, 10]], closed=True,
    ...
    def _draw_curling_view2(self, obs, agent_list):  # obs: [2, 100, 100] list
        ...
        for agent_idx in range(len(agent_list)):
            ...
            pygame.draw.lines(self.viewer.background,
                              points=[[549 + 60 * count, 0], [549 + 60 * count, 80], [549 + 80 + 60 * count, 80],
                                      [549 + 80 + 60 * count, 0]], closed=True,
                              color=COLORS[agent_list[agent_idx].color], width=2)
